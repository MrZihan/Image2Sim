...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            # num_rendered, color, feature_map, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args) ###############
            num_rendered, color, feature_map, depth, alpha, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args) ###d
        ...
        return color, feature_map, radii, depth, alpha

    @staticmethod
    ### def backward(ctx, grad_out_color, _):
    # def backward(ctx, grad_out_color, grad_out_feature, _): ###############
    def backward(ctx, grad_out_color, grad_out_feature, _, grad_depth, grad_alpha): ###d
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, semantic_feature = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None): ###d
        ...
        if semantic_feature is None:
            semantic_feature = torch.empty(0, device=device)
