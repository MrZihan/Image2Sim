...
def get_hm3d_pose_from_habitat(sensor_state):
    ...
    # Habitat World -> MP3D World (keep X, Y=-Z_hab, Z=Y_hab)
    R_hm3d_hab = np.array([
        [ 1,  0,  0],
        [ 0,  0, -1],
        [ 0,  1,  0]
    ...
    R_final = R_hm3d_hab @ R_hab_w_c
    pos_final = R_hm3d_hab @ pos_hab
...
def get_navigable_pcd_from_habitat(sim: habitat_sim.Simulator, grid_resolution: float = 0.05):
    ...
    valid_nav_points_hm3d = np.zeros_like(valid_nav_points)
    valid_nav_points_hm3d[:, 0] = valid_nav_points[:, 0]       
    valid_nav_points_hm3d[:, 1] = -valid_nav_points[:, 2]      
    valid_nav_points_hm3d[:, 2] = valid_nav_points[:, 1]       

    navigable_pcd = o3d.geometry.PointCloud()
    navigable_pcd.points = o3d.utility.Vector3dVector(valid_nav_points_hm3d)
...
def sample_and_render_panos(sim, habitat_nav_points, output_dir, grid_resolution=0.05, sqm_per_view=2.0, min_views=10, max_views=500):
    ...
    for i, pt in enumerate(tqdm(sampled_pts, desc="Rendering Panos")):
        ...
        # write_dpt(euclidean_depth, os.path.join(output_dir, f"{uuid}_depth.dpt"))
        ...
        t_hm3d, q_hm3d = get_hm3d_pose_from_habitat(sensor_state)
        pose_str = f"{t_hm3d[0]:.6f} {t_hm3d[1]:.6f} {t_hm3d[2]:.6f} " \
                   f"{q_hm3d[0]:.6f} {q_hm3d[1]:.6f} {q_hm3d[2]:.6f} {q_hm3d[3]:.6f}"
...
def save_pcd(nav_pcd, scene_pcd=None, scene_name="scene", save_path="hm3d", visualization=False):
...
def main():
    ...
    habitat_scene_path = "data/scene_datasets/hm3d/train"
    output_path = "data/nav_pcd/hm3d"
    ...
    for idx, scene_name in enumerate(scenes):
        hm3d_scene_path = habitat_scene_path+"/"+scene_name+"/"+scene_name.split("-")[-1]+".basis.glb"
        
        print(f"\n################################################################")
        print(f"Processing Scene {idx+1}/{len(scenes)}: {scene_name}")
        print(f"################################################################")

        cfg = make_habitat_sim_config(hm3d_scene_path)
        try:
            sim = habitat_sim.Simulator(cfg)
        except Exception as e:
            print(f"Failed to load scene {hm3d_scene_path}: {e}")
            exit()
